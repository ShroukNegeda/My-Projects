...
export default function RoleProfilePage({
  ...
  const getPhotoKey = (email) => email ? `eh_photo_${email}` : null;
  const [photo, setPhoto] = useState(() => { const k = getPhotoKey(userEmail); return k ? safeLS.getItem(k) || null : null; });
  useEffect(() => { const k = getPhotoKey(userEmail); setPhoto(k ? safeLS.getItem(k) || null : null); }, [userEmail]);
  const fileRef = useRef();
  const handlePhoto = (e) => {
    const file = e.target.files[0];
    if (!file || !userEmail) { if (!userEmail) alert("No email found"); return; }
    const k = `eh_photo_${userEmail}`;
    ...
    reader.onloadend = () => {
      ...
      img.onload = () => {
        ...
        try { safeLS.setItem(k, dataUrl); setPhoto(dataUrl); } catch { alert("Storage full."); }
      };
  ...
  const loadAcct = () => { const a = JSON.parse(safeLS.getItem("eh_accounts") || "{}"); return a[(userEmail || "").toLowerCase()] || {}; };
  const saveAcct = (patch) => {
    const a = JSON.parse(safeLS.getItem("eh_accounts") || "{}");
    a[(userEmail||"").toLowerCase()] = { ...loadAcct(), ...patch };
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          {isEditing ? (
            ...
            <div style={{ display:"flex", alignItems:"center", gap:10, justifyContent:"space-between" }}>
              <span style={{ fontSize:14, fontWeight:value?600:400, color:value?C.navy:"#94a3b8", lineHeight:1.4, flex: 1, minWidth: 0, overflowWrap: "anywhere" }}>{value||"Click edit to add"}</span>
  ...
  return (
    <SidebarLayout active={profileKey} setPage={setPage} userName={userName} userEmail={userEmail} userRole={userRole} logout={logout} photo={photo}>
      <style>{`
        ...
        @media (max-width: 992px) {
          .sidebar-container, [class*="sidebar-container"] {
            width: 80px !important;
            min-width: 80px !important;
            max-width: 80px !important;
            transition: width 0.3s ease !important;
          }
          .sidebar-container span { display: none !important; }
          .sidebar-container + div { margin-left: 80px !important; padding-left: 20px !important; }
        }
      `}</style>
