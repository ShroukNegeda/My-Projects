...
export default function RoleProfilePage({
  ...
  const EditRow = ({ fieldKey, label, icon, value, validate }) => {
    ...
    return (
      <div className="profile-field" style={{ display:"flex", alignItems:"center", gap:16, padding:"18px 0", borderBottom:`1px solid #f1f5f9`, transition:"background 0.2s", borderRadius:8, margin:"0 -8px", padding:"12px 8px" }}>
        <div className="field-icon" style={{ width:40, height:40, borderRadius:12, background:`linear-gradient(135deg, #fff7ed, #fff)`, border:"1.5px solid #ffe4cc", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, boxShadow:"0 2px 8px rgba(245,130,31,0.1)" }}>
          {icon}
        </div>
        <div style={{ flex:1, minWidth:0 }}>
          ...
          {isEditing ? (
            ...
            <div style={{ display:"flex", alignItems:"flex-start", gap:10, justifyContent:"space-between" }}>
              <span style={{ fontSize:14, fontWeight:value?600:400, color:value?C.navy:"#94a3b8", lineHeight:1.4, flex: 1, wordBreak: "break-word" }}>{value||"Click edit to add"}</span>
    ...
    <SidebarLayout active={profileKey} setPage={setPage} userName={userName} userEmail={userEmail} userRole={userRole} logout={logout} userPhoto={photo}>
      <style>{`
        ...
        @media (max-width: 640px) {
          .hero-content { flex-direction: column !important; text-align: center !important; gap: 14px !important; }
          .hero-card { padding: 24px 16px !important; border-radius: 20px !important; }
          .hero-info-stack { align-items: center !important; }
          .hero-avatar-box { width: 72px !important; height: 72px !important; border-radius: 18px !important; }
          .role-badge-row { flex-direction: column !important; align-items: center !important; text-align: center !important; gap: 6px !important; padding: 16px !important; }
          .role-badge-row div:last-child { margin-left: 0 !important; font-size: 10px !important; order: 3; margin-top: 4px; }
          .field-icon { width: 34px !important; height: 34px !important; border-radius: 10px !important; }
          .profile-field { gap: 10px !important; padding: 10px 4px !important; }
          h1 { font-size: 22px !important; }
        }

        @media (max-width: 992px) {
          .sidebar-container, [class*="sidebar-container"], .sidebar-nav, aside {
          ...
          .sidebar-container span, .sidebar-nav span, aside span { display: none !important; }
          .sidebar-container a, .sidebar-nav a, aside a, [class*="sidebar-link"] {
            justify-content: center !important;
            padding: 12px 0 !important;
          }
          .sidebar-container + div, .sidebar-container + main, aside + div {
      ...
      <div style={{ maxWidth:640, paddingBottom:80, margin: "0 auto" }}>
        ...
        <div className="hero-card" style={{ background:`linear-gradient(135deg, ${C.navy} 0%, #2d3580 100%)`, borderRadius:24, padding:"32px 32px 28px", marginBottom:20, boxShadow:"0 20px 60px rgba(26,31,94,0.25), 0 4px 16px rgba(26,31,94,0.15)" }}>
          <div className="hero-content" style={{ display:"flex", alignItems:"center", gap:24, position:"relative", zIndex:1 }}>
            {/* Avatar */}
            <div style={{ position:"relative", flexShrink:0 }}>
              <div className="hero-avatar-box" style={{ width:90, height:90, borderRadius:24, background:"linear-gradient(135deg, rgba(255,255,255,0.15), rgba(255,255,255,0.05))", border:"2px solid rgba(255,255,255,0.2)", display:"flex", alignItems:"center", justifyContent:"center", overflow:"hidden", boxShadow:"0 8px 32px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.1)" }}>
                {photo ? (
                  <img src={photo} alt="profile" style={{ width:"100%", height:"100%", objectFit:"cover", display: "block", borderRadius: "inherit" }}/>
                ) : (
            ...
            {/* Name + role */}
            <div className="hero-info-stack" style={{ flex:1, display: "flex", flexDirection: "column" }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap", justifyContent: "inherit" }}>
                ...
                <span style={{ fontSize:12, color:"rgba(255,255,255,0.6)", fontWeight:500 }}>{userEmail}</span>
              </div>
            </div>
          </div>
        ...
        <div style={{ background:"#fff", borderRadius:20, border:"1.5px solid #f1f5f9", boxShadow:"0 4px 24px rgba(26,31,94,0.06), 0 1px 4px rgba(26,31,94,0.04)", overflow:"hidden", marginBottom:16 }}>
          ...
          {/* Role badge row */}
          <div className="role-badge-row" style={{ padding:"16px 28px", borderTop:"1.5px solid #f1f5f9", background:"#fafbff", display:"flex", alignItems:"center", gap:14 }}>
