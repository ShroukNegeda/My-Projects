      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="mt-auto flex items-center justify-between pt-3">
          ...
          <div className="flex items-center gap-1 bg-laban-100 rounded-full">
            <button onClick={() => setQty((q) => Math.max(1, q - 1))} className="w-8 h-8 rounded-full flex items-center justify-center text-water-700 hover:bg-white transition-colors" aria-label="إنقاص الكمية">
              −
            </button>
            <span className="w-6 text-center font-semibold text-ink-900">{qty}</span>
            <button onClick={() => setQty((q) => q + 1)} className="w-8 h-8 rounded-full flex items-center justify-center text-water-700 hover:bg-white transition-colors" aria-label="زيادة الكمية">
              +
            </button>
          </div>
        </div>

        <button onClick={handleAdd} className={`mt-4 w-full py-2.5 rounded-full font-semibold transition-colors ${ added ? 'bg-emerald-500 text-white' : 'bg-water-600 text-white hover:bg-water-700'}`}>
          {added ? <><Check className="w-4 h-4 inline ml-1" />تمت الإضافة</> : 'أضيف للسلة'}
        </button>
      </div>
    </div>
  );
