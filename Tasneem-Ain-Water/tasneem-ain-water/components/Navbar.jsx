  ...
  return (
    <header className="sticky top-0 z-30 bg-laban-50/90 backdrop-blur border-b border-water-400/20">
      <nav className="container-page flex items-center justify-between h-20">
        <Link href="/" className="flex items-center gap-2 font-display text-2xl font-bold text-water-700">
          <DropletIcon className="w-7 h-7 text-water-500" />
          Tasneem Ain
        </Link>
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/cart"
            className="relative flex items-center justify-center w-11 h-11 rounded-full bg-white border border-water-400/30 hover:border-water-500 transition-colors"
            aria-label="السلة"
          >
            🛒
            {totalItems > 0 && (
          ...
          {user ? (
            <button
              onClick={logout}
              className="hidden sm:inline-flex px-4 py-2 rounded-full text-sm font-semibold text-water-700 hover:bg-water-100 transition-colors"
            >
              خروج
            </button>
          ) : (
            <Link
              href="/login"
              className="hidden sm:inline-flex px-4 py-2 rounded-full text-sm font-semibold bg-water-600 text-white hover:bg-water-700 transition-colors"
            >
              تسجيل الدخول
            </Link>
          )}
      ...
      {open && (
        <div className="md:hidden container-page pb-4 flex flex-col gap-3 text-ink-700 font-medium">
          ...
          {user ? (
            <button onClick={logout} className="text-right text-water-700">خروج</button>
          ) : (
